...
struct SurveyFisrt: View {
    let Question_one = ["경이", "육회비빔밥", "공부", "응아"]
    ...
    var body: some View {
                ...
                ForEach(Question_one.indices, id: \.self) { index in
                    Button(action: {
                        ...
                        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                            self.showAlert = true
                            self.isCorrectAnswer = (self.Question_one[index] == "경이")
        ...
        .alert(isPresented: $showAlert) {
            // 정답일 경우
            if isCorrectAnswer {
                return Alert(title: Text("정답"), message: Text("세상을"), dismissButton: .default(Text("확인"), action: {
