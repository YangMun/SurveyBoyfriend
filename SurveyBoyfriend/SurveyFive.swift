...
struct SurveyFive: View {
    let Question_one = ["힘두뤄..엉엉", "힘들면 세상 끝나? 그냥 해!!", "몰라 안훼!!!", "포기하고 다른 길로 가야하나"]
    ...
    var body: some View {
                ...
                ForEach(Question_one.indices, id: \.self) { index in
                    Button(action: {
                        ...
                        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                            self.showAlert = true
                            self.isCorrectAnswer = (self.Question_one[index] == "힘들면 세상 끝나? 그냥 해!!")
        ...
        .alert(isPresented: $showAlert) {
            // 정답일 경우
            if isCorrectAnswer {
                return Alert(title: Text("정답"), message: Text("아름답게"), dismissButton: .default(Text("확인"), action: {
    ...
    func updateButtonBackgroundColors(_ index: Int) {
        ...
        if Question_one[index] == "힘들면 세상 끝나? 그냥 해!!" {
