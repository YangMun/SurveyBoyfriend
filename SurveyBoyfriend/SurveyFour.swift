...
struct SurveyFour: View {
    let Question_one = ["대전", "부산", "서울", "미국"]
    ...
    var body: some View {
                ...
                ForEach(Question_one.indices, id: \.self) { index in
                    Button(action: {
                        ...
                        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                            self.showAlert = true
                            self.isCorrectAnswer = (self.Question_one[index] == "서울")
        ...
        .alert(isPresented: $showAlert) {
            // 정답일 경우
            if isCorrectAnswer {
                return Alert(title: Text("정답"), message: Text("고마워요"), dismissButton: .default(Text("확인"), action: {
