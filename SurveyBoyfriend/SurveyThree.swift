...
struct SurveyThree: View {
    let Question_one = ["호유동", "안양역", "안양 제2 소공원", "CGV"]
    ...
    var body: some View {
                ...
                ForEach(Question_one.indices, id: \.self) { index in
                    Button(action: {
                        ...
                        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                            self.showAlert = true
                            self.isCorrectAnswer = (self.Question_one[index] == "안양 제2 소공원")
        ...
        .alert(isPresented: $showAlert) {
            // 정답일 경우
            if isCorrectAnswer {
                return Alert(title: Text("정답"), message: Text("나의"), dismissButton: .default(Text("확인"), action: {
    ...
    func updateButtonBackgroundColors(_ index: Int) {
        ...
        if Question_one[index] == "안양 제2 소공원" {
