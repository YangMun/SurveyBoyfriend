...
struct SurveyTwo: View {
    let Question_one = ["5월 12일", "13월 29일", "1년내내", "10월 29일"]
    ...
    var body: some View {
                ...
                ForEach(Question_one.indices, id: \.self) { index in
                    Button(action: {
                        ...
                        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                            self.showAlert = true
                            self.isCorrectAnswer = (self.Question_one[index] == "10월 29일")
        ...
        .alert(isPresented: $showAlert) {
            // 정답일 경우
            if isCorrectAnswer {
                return Alert(title: Text("정답"), message: Text("만들어"), dismissButton: .default(Text("확인"), action: {
